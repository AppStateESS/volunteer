...
use volunteer\Exception\BadCommand;
use volunteer\Exception\PrivilegeMissing;
use volunteer\Factory\StoryMenu;
use phpws2\Database;
use Canopy\Request;
...
abstract class SubController
{
    ...
    public function __construct(\volunteer\Role\Base $role)
    {
        $this->role = $role;
    }
    ...
    protected function loadRequestId(Request $request)
    {
        ...
        if (!is_numeric($id)) {
            throw new \volunteer\Exception\ResourceNotFound($id);
        }
        $this->id = $id;
    }
    ...
    public function changeResponse(Request $request)
    {
        ...
        if ($request->isAjax() && in_array($method, ['post', 'put', 'patch', 'delete'])) {
